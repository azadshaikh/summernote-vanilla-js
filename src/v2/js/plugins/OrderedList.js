/**
 * OrderedListPlugin - Creates and manages numbered (ordered) lists
 * Handles <ol> elements with automatic numbering
 */
...
export default class OrderedListPlugin extends BasePlugin {
  ...
  /**
   * Initialize the OrderedList plugin
   */
  init() {
  ...
  handleKeydown(event) {
    ...
    if (!li) return; // only handle when caret is in an ordered list item

    // Only handle if we're inside an OL
    ...
    if (event.key === 'Backspace') {
      if (this.isListItemEmpty(li) || this.isCaretAtStartOfItem(li)) {
        ...
        const parentList = li.parentElement; // OL
  ...
  /**
   * Find the current LI element inside an OL containing the caret/selection
   */
  getCurrentListItem() {
    ...
    // Walk up to find LI inside OL
    while (node && node !== this.editor.editable) {
      if (node.tagName === 'LI' && node.parentElement && node.parentElement.tagName === 'OL') {
        return node;
      }
  ...
  indentListItem(li) {
    ...
    let nestedList = Array.from(prev.children).find(el => el.tagName === 'OL');
    if (!nestedList) {
   ...
   */
  updateButtonState() {
    const button = this.buttons.get('insertOrderedList');
   ...
   */
  destroy() {
    console.log('OrderedList plugin destroyed');
    super.destroy();
  }
}
